...
struct PagesSiteContractTests {
    ...
    @Test func builtSitePublishesTheRealScreenshotAndProductionFeedHistory() throws {
        ...
        #expect(feed.components(separatedBy: "<item>").count - 1 == 5)
        let currentRelease = try AppcastReleaseContract.validateFeed(feed, currentBuild: 5)
        #expect(currentRelease.version == 6)
        #expect(currentRelease.shortVersion == "0.2.1")
        #expect(currentRelease.archiveURL.absoluteString == "https://github.com/patrick-fu/agent-metrics-macos/releases/download/v0.2.1/AgentMetrics-0.2.1.dmg")
        #expect(currentRelease.archiveLength == 2_896_064)
